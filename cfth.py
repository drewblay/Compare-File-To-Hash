...
parser.add_argument('digest', help="The hash digest. Valid options: md5, sha256, sha512, sha1, sha224")
...
if hash_digest not in digests:
    print("First argument is not a valid hash digest, must be md5, sha256, sha512, sha1, sha224.")
    sys.exit(1)
elif file_to_hash.is_file() == False:
    print("Second argument ({}) is not a file.".format(file_to_hash))
    sys.exit(1)
else:
    file_hash = subprocess.run([digests[hash_digest], file_to_hash], stdout=subprocess.PIPE)
